...
.DESCRIPTION
    This PowerShell script automates the management of Active Directory (AD) users and groups. It defines user details, creates groups if they do not exist, and manages user accounts by deleting existing ones, creating new ones, and adding them to the appropriate groups.

.PARAMETER $password
    The password for all users, converted to a secure string.

.PARAMETER $users
    An array of user details, including first name, last name, username, and the group they belong to.

.PARAMETER $groups
    An array of groups and their respective Organizational Units (OUs).
...
.EXAMPLE
    .\Manage-ADUsersAndGroups.ps1
    This example runs the script to manage AD users and groups as defined in the script.
